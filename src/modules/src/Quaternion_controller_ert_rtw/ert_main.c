...
#include "Quaternion_controller.h"     /* Model's header file */
#include "rtwtypes.h"
...
void rt_OneStep(void)
{
  ...
  /* Check for overrun */
  if (OverrunFlag) {
    rtmSetErrorStatus(Quaternion_controller_M, "Overrun");
    return;
  }
...
int_T main(int_T argc, const char *argv[])
{
  ...
  fflush((NULL));
  while (rtmGetErrorStatus(Quaternion_controller_M) == (NULL)) {
  ...
  /* Terminate model */
  Quaternion_controller_terminate();
  return 0;
}
